...
// const dataFilePath = "./data/photos.json";
...
//   try {
//     const photos = JSON.parse(fs.readFileSync(dataFilePath, "utf-8"));
//     res.render('photo', { photos });
//   } catch (error) {
//     console.error("Error reading photo data:", error);
//     res.status(500).send("An error occurred while loading the album.");
//   }
// });
...
//     res.status(500).send("An error occurred while loading the upload page.");
//   }
// });
...
//     // Load existing photos and add the new photo
//     const photos = JSON.parse(fs.readFileSync(dataFilePath, "utf-8"));
//     photos.push({ name, relation, photoPath });
...
const upload = multer({ storage });
const familyTree = []; // Initialize with default or fetched data

// Routes
// Home page to display the family tree
app.get('/family', async (req, res) => {
  const familyTree = req.app.get('familyTree') || []; // Retrieve or initialize
  res.render('family.ejs', { familyTree });
});
...
app.post('/add-member', upload.single('photo'), (req, res) => {
  // Mock initialization of familyTree
  const familyTree = req.app.get('familyTree') || [];
  ...
  // Save familyTree globally or in a database
  req.app.set('familyTree', familyTree);
